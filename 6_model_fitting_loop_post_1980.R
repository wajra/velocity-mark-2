# Since ROMS data is available starting from 1980,
# we'll have to run a GAM for this period.

# Then we can compare the results from the GAMs and see
# if the results are still similar
...
# We also have to filter based on date
# For both 'dat' and 'hauls'

# Filtering 'dat' to have only post 1980 January 1 data
dat <- dat[(dat$year>=1980 & dat$month>=1),]

# Filtering 'hauls' to have only post 1980 January 1 data
hauls <- hauls[(hauls$year>=1980 & hauls$month>=1), ]
...
pdf(file=paste("figures/CEmodelGAMsmooths/GAMs_Revised_post_1980_Nov2017_GmexDrops_",runname,".pdf",sep=""),width=10,height=10)
...
for(i in 1:length(allspp)){  
  ...
  save(mods, file=paste(modfolder, 'CEmods_Nov2017_post_1980_GMEXdrop_', runname, '_', sp, '.RData', sep='')) 
  
  # write these files each time through the loop so that we can watch progress
  save(modeldiag, file=paste("output/modeldiag_Nov2017_post_1980_GMEXdrop_", runname,".Rdata",sep=""))
  write.csv(modeldiag, file=paste("output/modeldiag_Nov2017_post_1980_GMEXdrop_", runname,".csv",sep=""))
  
  write.csv(allwarnings, file=paste('output/warnings_Nov2017_post_1980_GMEXdrop_', runname, '.csv', sep=''))
}
dev.off()

# BE SURE TO DROP i=260 from modeldiag and allspp
modeldiag <- modeldiag[-260,]
save(modeldiag, file=paste("output/modeldiag_Nov2017_post_1980_", runname,".Rdata",sep=""))
write.csv(modeldiag, file=paste("output/modeldiag_Nov2017_post_1980_", runname,".csv",sep=""))

modeldiag_gmex <- modeldiag
rm(modeldiag)
load(paste("output/modeldiag_Nov2017_post_1980_", runname,".Rdata",sep=""))
