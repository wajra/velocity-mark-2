...
runname <- "uniform_null_model_2021_03_30"
...
# pred_obsMedianNosmear=n, pred_obsMeanNosmear=n, # tt is for training/testing model
...
for(i in 1:length(allspp)){  
  ...
  ########### ----------- Set up the random values ---------------- ###########
  # First for normal distribution
  spdata$null_norm <- rnorm(dim(spdata)[1], mean=10, sd=3)
  # Then for uniform distribution
  spdata$null_unif <- runif(dim(spdata)[1], min=0, max=10)
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ + s(null_unif))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(null_unif))
    'Presence-Absence model - Without temperature parameters'
    mynullpresmod<-formula(presfit ~ s(null_unif)) #Null model w/o temp
    'Abundance model - Without temperature parameters'
    mynullabunmod<-formula(logwtcpue.pad ~ s(null_unif)) #Null model w/o temp
  } else {
    print("We are going here")
    mypresmod<-formula(presfit ~ s(null_unif) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(null_unif) + regionfact)
    mynullpresmod<-formula(presfit ~ s(null_unif) + regionfact) #Null model w/o temp
    mynullabunmod<-formula(logwtcpue.pad ~ s(null_unif) + regionfact) #Null model w/o temp
  }
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(null_unif))
    myabunmodtt<-formula(logwtcpue.pad ~ s(null_unif))
  } else {
    mypresmodtt<-formula(presfit ~ s(null_unif) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(null_unif) + regionfact)
  }
