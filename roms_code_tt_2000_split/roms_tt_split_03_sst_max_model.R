...
runname <- "sst_max_model_2020_08_28"
...
for(i in 1:length(allspp)){  
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ s(SST.max) + s(rugosity) + s(GRAINSIZE))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(SST.max) + s(rugosity) + s(GRAINSIZE))
    ...
    mypresmod<-formula(presfit ~ s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(SST.max) + s(rugosity) + s(GRAINSIZE))
    myabunmodtt<-formula(logwtcpue.pad ~ s(SST.max) + s(rugosity) + s(GRAINSIZE))
  } else {
    mypresmodtt<-formula(presfit ~ s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
  }
