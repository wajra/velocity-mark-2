...
runname <- "random_null_model_2020_09_21"
...
for(i in 1:length(allspp)){  
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ + s(null_norm))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(null_norm))
    'Presence-Absence model - Without temperature parameters'
    mynullpresmod<-formula(presfit ~ s(null_norm)) #Null model w/o temp
    'Abundance model - Without temperature parameters'
    mynullabunmod<-formula(logwtcpue.pad ~ s(null_norm)) #Null model w/o temp
  } else {
    print("We are going here")
    mypresmod<-formula(presfit ~ s(null_norm) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(null_norm) + regionfact)
    mynullpresmod<-formula(presfit ~ s(null_norm) + regionfact) #Null model w/o temp
    mynullabunmod<-formula(logwtcpue.pad ~ s(null_norm) + regionfact) #Null model w/o temp
  }
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(null_norm))
    myabunmodtt<-formula(logwtcpue.pad ~ s(null_norm))
  } else {
    mypresmodtt<-formula(presfit ~ s(null_norm) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(null_norm) + regionfact)
  }
