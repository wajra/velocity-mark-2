# ROMS - A new null model???
# Run the models using both a random uniform distribution and random normal
# distribution inplace of the older null model
...
filtered_hauls_roms <- read.csv('data/transformed_haul_data_NWA-SZ_HCob10T_including_column_ver_3.csv',stringsAsFactors = FALSE)
...
# Here we will drop the following regions - 'DFO_ScotianShelf', 'DFO_SoGulf'
# Print out the unique regions
print(unique(hauls$region))
# Here we are dropping the regions
hauls <- hauls[!(hauls$region %in% c("DFO_ScotianShelf", "DFO_SoGulf")),]

# Now we'll list the seasons
print(unique(hauls$survey))

# Changed the runname for my own diagnostics
runname <- "mean_null_model_2020_09_22"
...
for(i in 1:length(allspp)){  
  ...
  spdata$mean_grainsize <- mean(spdata$GRAINSIZE, na.rm=TRUE)
  spdata$mean_rugosity <- mean(spdata$rugosity, na.rm=TRUE)
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ + s(mean_rugosity) + s(mean_grainsize))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(mean_rugosity) + s(mean_grainsize))
    'Presence-Absence model - Without temperature parameters'
    mynullpresmod<-formula(presfit ~ s(mean_rugosity) + s(mean_grainsize)) #Null model w/o temp
    'Abundance model - Without temperature parameters'
    mynullabunmod<-formula(logwtcpue.pad ~ s(mean_rugosity) + s(mean_grainsize)) #Null model w/o temp
  } else {
    print("We are going here")
    mypresmod<-formula(presfit ~ s(mean_rugosity) + s(mean_grainsize) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(mean_rugosity) + s(mean_grainsize) + regionfact)
    mynullpresmod<-formula(presfit ~ s(mean_rugosity) + s(mean_grainsize) + regionfact) #Null model w/o temp
    mynullabunmod<-formula(logwtcpue.pad ~ s(mean_rugosity) + s(mean_grainsize) + regionfact) #Null model w/o temp
  }
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(mean_rugosity) + s(mean_grainsize))
    myabunmodtt<-formula(logwtcpue.pad ~ s(mean_rugosity) + s(mean_grainsize))
  } else {
    mypresmodtt<-formula(presfit ~ s(mean_rugosity) + s(mean_grainsize) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(mean_rugosity) + s(mean_grainsize) + regionfact)
  }
