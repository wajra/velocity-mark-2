...
hindcast_1985$lat_grid05 <- floor(hindcast_1985$lat*2)/2 + 0.25
hindcast_1985$lon_grid05 <- floor(hindcast_1985$lon*2)/2 + 0.25

sum <- aggregate(log(hindcast_1985$preds+1), 
                 by = list(lat = hindcast_1985$lat_grid05,
                           lon = hindcast_1985$lon_grid05),
                 FUN = mean, na.rm= TRUE)
...
test_plot <- ggplot(data = world) +
  ...
  coord_sf(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = FALSE) +
  labs(fill="log(Biomass+1)") + ylab("Latitude") + xlab("") + 
  geom_tile(data=sum_abundance, aes(lon, lat, width = 0.5, fill = x)) +
...
plot_2 <- ggplot(data = world) +
  geom_sf() +
  coord_sf(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = FALSE) +
  geom_tile(data=sum, aes(lon, lat, width=0.5, fill=x)) + 
...
png("roms_figures_output_ver_1/classic_model_predictions_large_grid.png", width=6, height=6, units="in", res=300)
