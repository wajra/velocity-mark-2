# Visualizing Interactions - Part I
# Get means/modes for environmental parameters for black sea bass observations
...
# Load species catch data
load('data/dat_selectedspp_Feb_1_2017.Rdata')

# Read the ROMS data for all hauls in NWA
filtered_hauls_roms <- read.csv('data/transformed_haul_data_NWA-SZ_HCob10T_including_column_ver_4.csv',
  stringsAsFactors = FALSE)

# Drop some of the unncessary columns
filtered_hauls_roms <- subset(filtered_hauls_roms, 
  select=-c(sppocean,year,month,lon,lat))

# Now we merge the hauls with the ROMS dataset
hauls <- merge(filtered_hauls_roms,hauls,by='haulid',all.x=T,sort=F)

# Changed the runname for my own diagnostics
# runname <- "salinity_model_2020_08_26"

# Before all other data clean ups we can actually filter out only black 
# sea bass columns
...
# Here they simply say that all other rows apart from those having a 'wtcpue' 
# value of 0 and 'region'
# value of 'DFO_SoGulf' are actual absences therefore can be removed 
# from the dataframe
# the zeros in SoGulf are actual zeros (ie not just a scale issue) and thus 
# are true absences
dat <- dat[!(dat$wtcpue == 0 & dat$region == 'DFO_SoGulf'),] 
# And then the remaining zero values of 'wtcpue' are set to 0.0002 because 
# they do have a value. They are not absences.
# They are barely there
dat$wtcpue[dat$wtcpue == 0] <- 0.0002 # 'zeros' in dat are now species too 
# light to register on scales_here a value below the lowest non-zero value is 
# assigned_for transforming data
# Then we check for the species ('sppocean' column) named 'calappa sulcata in 
# Atlantic Ocean' and
# check if there 'wtcpue' is missing. If so we set it to 0.13.
# the median weight of this species when observed
dat$wtcpue[dat$sppocean=="calappa sulcata_Atl" & is.na(dat$wtcpue)] <- 0.13
# Then a new column is created named 'logwtcpue' and log values of 'wtcpue' 
# is set to it
dat$logwtcpue <- log(dat$wtcpue)

# drop dubious GMex observations of temperate species or species that have a 
# Gulf of Mexico endemic congener that closely resembles the Atlantic coast 
# species
# So what we have here is a list or vector of a set of species that are 
# in both Gulf of Mexico and Atlantic Ocean
drops <- c('alosa pseudoharengus_Gmex', 'brevoortia tyrannus_Gmex', 
  'clupea harengus_Gmex', 'dipturus laevis_Gmex', 'paralichthys dentatus_Gmex',
  'hippoglossoides platessoides_Gmex', 'pseudopleuronectes americanus_Gmex', 
  'scomber scombrus_Gmex', 'cryptacanthodes maculatus_Gmex',
           'echinarachnius parma_Gmex', 'illex illecebrosus_Gmex',
  'melanostigma atlanticum_Gmex', 'menidia menidia_Gmex', 
  'ovalipes ocellatus_Gmex','placopecten magellanicus_Gmex')
# 'gsub' is a function that simply substitutes a substring for another 
# substring in a string or vector
# USAGE
# gsub(pattern, replacement, x, ignore.case = FALSE, perl = FALSE, 
# fixed = FALSE, useBytes = FALSE)
drops <- gsub('_Gmex', '_Atl', drops)

# OK. So here we need to understand some context. 'SEFSC_GOMexFall' is the 
# Gulf of Mexico survey
# This survey went from 1982-2014 and occured in the Summer and Fall.
# So what needs to be done is see whether a row has 'region' belonging to 
# these surveys and seasons and
...
# trim columns that are already in master hauls file, which will be merged in 
# below with the hauls data
# Creating a new data frame using columns and data from 'dat' data frame
dat <- data.frame(haulid = dat$haulid, sppocean = dat$sppocean, 
  Freq = dat$Freq, wtcpue = dat$wtcpue, logwtcpue = dat$logwtcpue, 
  presfit = TRUE, stringsAsFactors = F)
# Here the rows that have'sppocean'  value of 'NO CATCH' will be filtered out
dat <- dat[!dat$sppocean=='NO CATCH',]
#dat <- dat[!is.na(dat$wtcpue),] # drop NAs as it creates errors for some 
# species. May want to go back and manually do 'oncorhynchus tshawytscha_Pac' 
# as almost 10% of presence records have NA for wtcpue (tagging study?)
# Found a species naming error_correcting here
dat$sppocean[dat$sppocean=='heppasteria phygiana_Atl'] <- 
  'hippasteria phrygiana_Atl' 
# no need to aggregate as they never overlapped 
# w/n hauls (only found in Newfoundland and they called it one way or the other)
...
bsb_env <- bsb_env %>% 
  ...
  regionfact, zplk_class_mean_seasonal)
...
# Let's get one of the regions
bsb_vims <- bsb_env %>% 
  filter(regionfact=="VIMS_NEAMAP")
...
bsb_matrix <- expand.grid(SBT.min=mean(bsb_env$SBT.min),
  ...
  zplk_class_mean_seasonal=mean(bsb_env$zplk_class_mean_seasonal),
...
hauls <- hauls[!(hauls$region %in% c("DFO_ScotianShelf", "DFO_SoGulf")),]

# Changed the runname for my own diagnostics
runname <- "visualizing_interactions_mi_model_2021_02_02"
...
for(i in 1:length(allspp)){  
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE))
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE))
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE))
  } else {
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(o2_seasonal) + s(SBT.seasonal, o2_seasonal) + s(zplk_class_mean_seasonal) + s(SST.seasonal.mean) + s(SBT.min) + s(SBT.max) + s(SST.max) + s(rugosity) + s(GRAINSIZE) + regionfact)
  }
...
ggsave("roms_figures_output_ver_1/abundance_plot_ver_mi.png", abundance_plot, 
       width = 6,
       height = 6,
       units = c("in"),
       dpi = 300,)
...
ggsave("roms_figures_output_ver_1/abundance_se_plot_ver_mi.png", abundance_se_plot, 
       width = 6,
       height = 6,
       units = c("in"),
       dpi = 300,)
...
ggsave("roms_figures_output_ver_1/presence_plot_ver_mi.png", presence_plot, 
       width = 6,
       height = 6,
       units = c("in"),
       dpi = 300,)
...
# 
ggsave("roms_figures_output_ver_1/presence_se_plot_ver_mi.png", presence_se_plot, 
       width = 6,
       height = 6,
       units = c("in"),
       dpi = 300,)
...
#### Write these into files
write.csv(full_model_predictions_abundance_val, file="visualizing_interactions/abundance_predictions.csv")
write.csv(full_model_predictions_presence_val, file="visualizing_interactions/presence_predictions.csv")
write.csv(full_model_predictions_abundance_se, file="visualizing_interactions/abundance_se_predictions.csv")
write.csv(full_model_predictions_presence_se, file="visualizing_interactions/presence_se_predictions.csv")



# Part II : Then for the testing/training split derived GAM
presence_tt_prediction <- predict(mygam1tt, newdata=bsb_matrix, 
  type='response', na.action='na.pass')



# After this, we answer the question if this model improves decadal forecasting
# capability
# First do for the decade of 60's. Use that to predict the 70's and so on...

# For this we need to read in the merged dataset that we saved to the disk

decadal_df <- read.csv(file="data/merged_bsb_observations.csv", 
  stringsAsFactors = FALSE)

decade_1960s_df <- decadal_df %>% filter(between(decadal_df$year, 1960, 1970))

# 2020/11/19
# Next thing I want to do is to create a count of observations for each year
# So that I can think of the better intervals/gaps for decadal forecasting

# We'll do this in the next R script
# vis_interaction_part_ii_decadal_forecasting.R